...
Ref<SpineAnimationAttri> SpineAnimation::get_attri() const
{
	...
	/*spSkeletonData* skelData = state->data->skeletonData;
	String hintString = skelData->animations[0]->name;
	for (int i = 1; i < skelData->animationsCount; ++i)
	{
		hintString += ", ";
		hintString += skelData->animations[i]->name;
	}
	ret->set_animation_hint_string(hintString);*/
